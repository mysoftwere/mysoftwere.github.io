...
function getSheet_() {
  ...
  if (!sh) {
    ...
    sh.getRange(1, 1, 1, 6).setValues([
      ["id", "name", "url", "category", "description", "position"]
    ]);
  }
...
function doPost(e) {
  try {
    ...
    if (data.action === "add") {
      result = addSoftware_(data.item);

    } else if (data.action === "edit") {
      result = editSoftware_(data.id, data.item);

    } else if (data.action === "delete") {
      result = deleteSoftware_(data.id);

    } else if (data.action === "reorder") {
      result = reorderSoftware_(data.ids);
...
function addSoftware_(item) {
  if (!item || !item.id || !item.name || !item.url) {
    return {
      success: false,
      error: "Name, URL and ID are required"
    };
  }
  ...
  const pos = list.length
    ? Math.max.apply(
        null,
        list.map(x => Number(x.position) || 0)
      ) + 1
    : 1;
...
function reorderSoftware_(ids) {
  ...
  ids.forEach((id, i) => {
    ...
    if (row !== -1) {
      sh.getRange(row, 6).setValue(i + 1);
    }
  });
